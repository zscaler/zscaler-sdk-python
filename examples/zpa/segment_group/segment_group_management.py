...
def main():
    parser = argparse.ArgumentParser(
        description="Manage Segment Groups for a Cloud Service Provider."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="List all segment groups"
    )
    parser.add_argument(
        "-g", "--get", metavar="GROUP_ID", help="Get details of a segment group by ID"
    )
    parser.add_argument(
        "-n",
        "--get_by_name",
    ...
    parser.add_argument(
        "-d", "--delete", metavar="GROUP_ID", help="Delete a segment group by ID"
    )
    parser.add_argument("--add", action="store_true", help="Add a new segment group")
    parser.add_argument(
        "--update", metavar="GROUP_ID", help="Update an existing segment group"
    )
    parser.add_argument("--name", help="Name of the segment group")
    parser.add_argument(
        "--enabled", type=str2bool, help="Whether the segment group is enabled"
    )
    ...
    if args.list:
        ...
        print(
            json.dumps(segment_group, indent=4)
            if segment_group
            else f"No segment group found with ID {args.get}"
        )

    elif args.get_by_name:
        segment_group = client.segment_groups.get_segment_group_by_name(
            args.get_by_name
        )
        print(
            json.dumps(segment_group, indent=4)
            if segment_group
            else f"No segment group found with name {args.get_by_name}"
        )
        ...
        new_group = client.segment_groups.add_group(
            name=args.name, enabled=args.enabled, description=args.description
        )
        ...
        print(
            f"Segment Group {args.update} updated successfully: {json.dumps(updated_group, indent=4)}"
        )
