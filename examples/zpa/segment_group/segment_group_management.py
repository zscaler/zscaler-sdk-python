...
def main():
    ...
    parser.add_argument("-n", "--get_by_name", metavar="GROUP_NAME", help="Get details of a segment group by name")
    ...
    if args.list:
        ...
        print(f"Segment Group {args.delete} deleted successfully." if response_code == 204 else f"Failed to delete Segment Group {args.delete}. Response code: {response_code}")
        ...
        updated_group = client.segment_groups.update_group(group_id=args.update, name=args.name, enabled=args.enabled, description=args.description)
