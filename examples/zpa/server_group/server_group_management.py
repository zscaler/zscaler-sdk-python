...
def main():
    ...
    parser.add_argument("-n", "--get_by_name", metavar="NAME", help="Get details of a server group by name.")
    ...
    parser.add_argument("--dynamic_discovery", type=str2bool, help="Whether dynamic discovery is enabled")
    ...
    parser.add_argument("--server_ids", help="Server IDs (comma-separated, required if dynamic discovery is false)")
...
def add_server_group(client, args):
    ...
    enabled = args.enabled if args.enabled is not None else input("Is the server group enabled? (True/False): ").lower() == 'true'
    app_connector_group_ids = args.app_connector_group_ids.split(',') if args.app_connector_group_ids else input("Enter App Connector Group IDs (comma-separated): ").split(',')
    server_ids = args.server_ids.split(',') if args.server_ids else None if dynamic_discovery else input("Enter Server IDs (comma-separated, required if dynamic discovery is false): ").split(',')
