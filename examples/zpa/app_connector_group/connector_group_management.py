...
def main():
    ...
    parser.add_argument("-n", "--get_by_name", metavar="GROUP_NAME", help="Get details of a connector group by name")
    ...
    if args.list:
        ...
        print(json.dumps(connector_group, indent=4) if connector_group else f"No connector group found with name {args.get_by_name}")

    elif args.delete:
        response_code = client.connectors.delete_connector_group(args.delete)
        print(f"Connector Group {args.delete} deleted successfully." if response_code == 204 else f"Failed to delete Connector Group {args.delete}. Response code: {response_code}")

    elif args.add:
        new_group = client.connectors.add_connector_group(name=args.name, latitude=args.latitude, longitude=args.longitude, location=args.location)
        print(f"Connector Group added successfully: {json.dumps(new_group, indent=4)}")

    elif args.update:
        updated_group = client.connectors.update_connector_group(group_id=args.update, name=args.name, latitude=args.latitude, longitude=args.longitude, location=args.location)
