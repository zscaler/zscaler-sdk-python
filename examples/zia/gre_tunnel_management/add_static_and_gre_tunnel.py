...
def main():
    parser = argparse.ArgumentParser(
        description="CLI tool to add a GRE tunnel in Zscaler Internet Access."
    )
    parser.add_argument(
        "--ip_address",
        required=True,
    ...
    parser.add_argument(
        "--location_name", required=True, help="Name for the new location."
    )
    ...
    configure_gateway_options = prompt_yes_no(
        "Would you like to configure gateway options for the location?"
    )
    gateway_options = {}
    ...
    static_ip_response = zia.traffic.add_static_ip(
        args.ip_address, comment=args.comments
    )
    ...
    gre_tunnel_response = zia.traffic.add_gre_tunnel(
        source_ip=args.ip_address, comment=args.comments
    )
    print(
        f"GRE Tunnel created successfully: {json.dumps(gre_tunnel_response, indent=4)}"
    )
