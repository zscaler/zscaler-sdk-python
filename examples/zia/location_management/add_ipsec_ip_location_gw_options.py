...
def add_location_with_ip_tunnel(zia, name, ip_address, pre_shared_key, gateway_options):
    ...
    print(f"IPSec IP tunnel created with ID: {vpn_credential_response.get('id')} Response: {json.dumps(vpn_credential_response, indent=4)}")
...
def main():
    parser = argparse.ArgumentParser(description="Manage locations in Zscaler Internet Access with an associated IPSec tunnel (IP type).")
    parser.add_argument("--add_location", action="store_true", help="Add a new location with an IPSec IP tunnel.")
    ...
    if args.add_location and args.name and args.ip_address:
        ...
        print("Missing required arguments: --name and --ip_address are required for adding a location with an IPSec IP tunnel.")
