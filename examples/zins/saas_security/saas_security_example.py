...
This example demonstrates how to query SaaS Security (CASB) analytics data
from Z-Insights using the Zscaler Python SDK.

Usage:
    python saas_security_example.py [--limit <number>] [--days <number>]
...
def main():
    ...
    print(f"Querying SaaS Security data for the last {args.days} days...")

    with ZscalerClient(config) as client:
        # Query CASB application report
        print_header("CASB Application Report")
        entries, response, error = client.zins.saas_security.get_casb_app_report(
