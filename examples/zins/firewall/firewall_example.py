...
This example demonstrates how to query Zero Trust Firewall analytics from
Z-Insights using the Zscaler Python SDK.

Usage:
    python firewall_example.py [--limit <number>] [--days <number>]
...
def main():
    ...
    print(f"Querying Zero Trust Firewall data for the last {args.days} days...")

    with ZscalerClient(config) as client:
        # Query traffic by action
        print_header("Firewall Traffic by Action")
        entries, response, error = client.zins.firewall.get_traffic_by_action(
        ...
        if error:
            ...
            for entry in entries:
                print(f"  {entry.get('name', 'Unknown')}: {entry.get('total', 0):,}")
        else:
            print("  No firewall action data available.")

        # Query traffic by location
        print_header("Firewall Traffic by Location")
        entries, response, error = client.zins.firewall.get_traffic_by_location(
            start_time=start_time, end_time=end_time, limit=args.limit
        )
        ...
        entries, response, error = client.zins.firewall.get_network_services(
            start_time=start_time, end_time=end_time, limit=args.limit
        )
        ...
        if error:
            ...
            print("  No network services data available.")
