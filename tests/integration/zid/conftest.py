...
class MockZIdClient(ZscalerClient):
    def __init__(self, fs, config=None):
        """
        Initialize the MockZIdClient with support for environment variables and
...
@pytest.fixture
def zid_client(fs):
    """Return a Zscaler client for Z Id (Z Identity admin) integration tests."""
    return MockZIdClient(fs)
