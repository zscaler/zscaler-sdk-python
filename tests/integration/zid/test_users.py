...
class TestUsers:
    ...
    @pytest.mark.vcr()
    def test_users(self, zid_client):
        client = zid_client
        errors = []
        user_id = None
        update_user = None
        ...
        try:
            ...
            try:
                if update_user:
                    user, _, error = client.zid.users.get_user(update_user.id)
            ...
            try:
                if update_user:
                    users_response, _, error = client.zid.users.list_users()
            ...
            if user_id:
                try:
                    delete_response, _, err = client.zid.users.delete_user(user_id)
