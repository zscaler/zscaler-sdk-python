...
class TestGroups:
    ...
    @pytest.mark.vcr()
    def test_groups(self, zid_client):
        client = zid_client
        errors = []
        group_id = None
        update_group = None
        ...
        try:
            # Test: Add Group
            try:
                create_group, _, error = client.zid.groups.add_group(
            ...
            try:
                if group_id:
                    update_group, _, error = client.zid.groups.update_group(
                        group_id=group_id,
            ...
            try:
                if update_group:
                    group, _, error = client.zid.groups.get_group(update_group.id)
            ...
            try:
                if update_group:
                    groups_response, _, error = client.zid.groups.list_groups(query_params={"search": update_group.name})
            ...
            try:
                if update_group:
                    _, _, error = client.zid.groups.delete_group(update_group.id)
