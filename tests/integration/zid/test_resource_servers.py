...
class TestResourceServers:
    ...
    @pytest.mark.vcr()
    def test_resource_servers(self, zid_client):
        client = zid_client
        ...
        try:
            resource_response, _, err = client.zid.resource_servers.list_resource_servers()  # Correctly unpack the tuple
        ...
        if resource_id:
            ...
            try:
                fetched_resource, _, err = client.zid.resource_servers.get_resource_server(resource_id)
                ...
                assert (
                    fetched_resource.id == resource_id
                ), "Mismatch in Resource Server ID"  # Use dot notation for object access
            except Exception as exc:
