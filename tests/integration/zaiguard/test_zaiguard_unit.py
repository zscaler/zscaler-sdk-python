...
class TestPolicyDetectionUnit:
    ...
    def test_execute_policy_request_error(self, fs):
        ...
        result, response, err = policy_api.execute_policy(content="test", direction="IN")
    ...
    def test_resolve_and_execute_policy_request_error(self, fs):
        ...
        policy_api = PolicyDetectionAPI(mock_executor)
        result, response, err = policy_api.resolve_and_execute_policy(content="test", direction="IN")
...
class TestModelsUnit:
    ...
    def test_rate_limit_throttling_detail_creation(self, fs):
        ...
        # Test with data
        throttle = RateLimitThrottlingDetail({"rlcId": 12345, "metric": "rq", "retryAfterMillis": 5000})
    ...
    def test_execute_policy_request_creation(self, fs):
        ...
        request_obj = ExecuteDetectionsPolicyRequest(
            {"transactionId": "abc-123", "content": "test content", "direction": "IN", "policyId": 12345}
        )
        ...
        response_obj = ExecuteDetectionsPolicyResponse(
            {
                ...
                "severity": "LOW",
                "direction": "OUT",
                "detectorResponses": {"detector1": {"statusCode": 200, "triggered": False, "action": "ALLOW"}},
                "throttlingDetails": [{"rlcId": 999, "metric": "rq", "retryAfterMillis": 1000}],
            }
        )
...
class TestLegacyClientUnit:
    ...
    def test_legacy_client_custom_url(self, fs):
        ...
        client = LegacyZGuardClientHelper(api_key="test_api_key", cloud="us1", override_url=custom_url)
    ...
    def test_legacy_client_rate_limit_config(self, fs):
        ...
        client = LegacyZGuardClientHelper(
            api_key="test_api_key", cloud="us1", auto_retry_on_rate_limit=False, max_rate_limit_retries=5
        )
    ...
    def test_handle_throttling_details_rq_metric(self, fs):
        ...
        client = LegacyZGuardClientHelper(
            api_key="test_api_key", cloud="us1", auto_retry_on_rate_limit=False  # Disable auto-retry for testing
        )

        throttle = RateLimitThrottlingDetail({"rlcId": 123, "metric": "rq", "retryAfterMillis": 5000})
    ...
    def test_handle_throttling_details_cs_metric(self, fs):
        ...
        throttle = RateLimitThrottlingDetail({"rlcId": 456, "metric": "cs", "retryAfterMillis": 3000})
    ...
    def test_handle_throttling_details_multiple(self, fs):
        ...
        throttles = [
            RateLimitThrottlingDetail({"rlcId": 111, "metric": "rq", "retryAfterMillis": 5000}),
            RateLimitThrottlingDetail({"rlcId": 222, "metric": "cs", "retryAfterMillis": 3000}),
        ]
...
class TestRateLimitingLogic:
    ...
    def test_thread_safety(self, fs):
        ...
        def increment_throttles():
            throttle = RateLimitThrottlingDetail({"metric": "rq", "retryAfterMillis": 100})
