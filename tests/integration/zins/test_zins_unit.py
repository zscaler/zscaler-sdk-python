...
class TestShadowItUnit:
    ...
    def test_get_apps_request_error(self, fs):
        ...
        result, response, err = shadow_api.get_apps(
            start_time=int(time.time() * 1000) - 86400000, end_time=int(time.time() * 1000)
        )
    ...
    def test_get_apps_graphql_error(self, fs):
        ...
        mock_response = Mock()
        mock_response.get_body = Mock(return_value={"errors": [{"message": "GraphQL error"}]})
    ...
    def test_extract_graphql_response_exception(self, fs):
        ...
        result, response, err = shadow_api._extract_graphql_response(mock_response, "http://test.com", "SHADOW_IT", "apps")
