...
class TestCBIBanners:
    ...
    def test_cbi_banner(self, fs):
        ...
        try:
            ...
            assert err is None, f"Error creating banner: {err}"
            assert created_banner is not None, "Banner creation returned None"
            # assert created_banner.name == banner_name

            banner_id = created_banner.id
            assert banner_id is not None, "Banner ID was not returned on creation"

            banner_id = created_banner.id
        except Exception as exc:
            errors.append(f"Error during cbi banner creation: {exc}")

        try:
            if banner_id:
                # Step 2: Retrieve the banner by ID
                retrieved_banner, _, err = client.zpa.cbi_banner.get_cbi_banner(banner_id)
                assert err is None, f"Error fetching CBI banner: {err}"
                assert retrieved_banner is not None, "Retrieved banner is None"
                assert retrieved_banner.id == banner_id
                # assert retrieved_banner.name == banner_name

                # Step 3: List all banners and confirm our banner exists in the list
                banners_list, _, err = client.zpa.cbi_banner.list_cbi_banners()
                assert err is None, f"Error listing banners: {err}"
                assert any(banner.id == banner_id for banner in banners_list), "Created banner not found in banner list"
            ...
            errors.append(f"CBI Banner operation failed: {exc}")

        finally:
            if banner_id:
                try:
                    # Step 5: Delete the banner
                    delete_response, _, err = client.zpa.cbi_banner.delete_cbi_banner(banner_id)
                    assert err is None, f"Error deleting banner: {err}"
                    assert delete_response is None, "Expected None for 204 No Content response"

                except Exception as cleanup_exc:
                    errors.append(f"Cleanup failed for CBI Banner ID {banner_id}: {cleanup_exc}")

        # Assert that no errors occurred during the test
        assert len(errors) == 0, f"Errors occurred during the CBI Banner lifecycle test: {errors}"
