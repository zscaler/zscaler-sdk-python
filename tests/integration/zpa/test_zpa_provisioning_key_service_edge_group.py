...
class TestServiceEdgeGroupProvisioningKey:
    ...
    async def test_provisioning_key_operations(self, fs): 
        ...
        try:
            ...
            svc_edge_group_cert = client.certificates.get_enrolment_cert_by_name("Service Edge")
            ...
            assert any(key['id'] == svc_edge_group_key_id for key in all_svc_edge_group_keys), "Service Edge Group key not found in list"

            # Test retrieving the specific SERVICE_EDGE_GRP Provisioning Key
            retrieved_svc_edge_group_key = client.provisioning.get_provisioning_key(svc_edge_group_key_id, "service_edge")
            assert retrieved_svc_edge_group_key['id'] == svc_edge_group_key_id, "Failed to retrieve the correct SERVICE_EDGE_GRP Provisioning Key"
            ...
            assert updated_svc_edge_key['max_usage'] == '3', "Failed to update maxUsage for SERVICE_EDGE_GRP Provisioning Key"
            
            # Cleanup: Delete the SERVICE_EDGE_GRP Provisioning Key
            delete_status_svc_edge = client.provisioning.delete_provisioning_key(svc_edge_group_key_id, "service_edge")
            assert delete_status_svc_edge == 204, "Failed to delete SERVICE_EDGE_GRP Provisioning Key"
            svc_edge_group_key_id = None  # Reset ID after deletion to prevent cleanup attempt

        except Exception as exc:
            errors.append(f"Operations on SERVICE_EDGE_GRP Provisioning Key failed: {exc}")

        # Cleanup
        try:
            if svc_edge_group_key_id:
                client.provisioning.delete_provisioning_key(svc_edge_group_key_id, "service_edge")
            if group_id:
