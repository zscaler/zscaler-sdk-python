...
class TestZPAGateway:
    ...
    def test_zpa_gateway_crud(self, fs):
        ...
        try:
            ...
            # Test get_gateway with first gateway if available - may fail if ZPA tenant not configured
            if gateways and len(gateways) > 0:
                gateway_id = gateways[0].id
                fetched_gateway, response, err = client.zia.zpa_gateway.get_gateway(gateway_id)
                # May return 404 if ZPA tenant not properly configured
                if err is None:
