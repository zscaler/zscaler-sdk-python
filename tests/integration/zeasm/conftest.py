...
def zeasm_client(fs):
    return MockZEASMClient(fs)


class MockZEASMClient(ZscalerClient):
    def __init__(self, fs, config=None):
        """
        Initialize the MockZEASMClient with support for environment variables and
        ...
        # ZEASM uses ZEASM_* env vars with fallback to ZSCALER_* for consistency
        # In playback mode (MOCK_TESTS=true), use dummy credentials if not provided
        clientId = config.get("clientId", os.getenv("ZEASM_CLIENT_ID", os.getenv("ZSCALER_CLIENT_ID")))
        clientSecret = config.get("clientSecret", os.getenv("ZEASM_CLIENT_SECRET", os.getenv("ZSCALER_CLIENT_SECRET")))
        customerId = config.get("customerId", os.getenv("ZEASM_CUSTOMER_ID", os.getenv("ZPA_CUSTOMER_ID")))
        vanityDomain = config.get("vanityDomain", os.getenv("ZEASM_VANITY_DOMAIN", os.getenv("ZSCALER_VANITY_DOMAIN")))
