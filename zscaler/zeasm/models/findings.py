...
class Findings(ZscalerObject):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if config:
            self.results = ZscalerCollection.form_list(config["results"] if "results" in config else [], FindingDetails)
...
class FindingDetails(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.cisa_likelihood = config["cisa_likelihood"] if "cisa_likelihood" in config else None
            self.country = config["country"] if "country" in config else None
            self.description = config["description"] if "description" in config else None
            self.epss_likelihood = config["epss_likelihood"] if "epss_likelihood" in config else None
            self.first_seen = config["first_seen"] if "first_seen" in config else None
            self.id = config["id"] if "id" in config else None
            self.impacted_asset_id = config["impacted_asset_id"] if "impacted_asset_id" in config else None
            self.impacted_asset_name = config["impacted_asset_name"] if "impacted_asset_name" in config else None
            self.is_stale = config["is_stale"] if "is_stale" in config else None
            self.last_seen = config["last_seen"] if "last_seen" in config else None
            self.name = config["name"] if "name" in config else None
            self.profile_id = config["profile_id"] if "profile_id" in config else None
            self.risk_level = config["risk_level"] if "risk_level" in config else None
            self.risk_score = config["risk_score"] if "risk_score" in config else None
            self.scan_type = config["scan_type"] if "scan_type" in config else None
            self.severity_score = config["severity_score"] if "severity_score" in config else None
...
class CommonFindings(ZscalerObject):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if config:
            self.content: Optional[Any] = config["content"] if "content" in config else None
            self.source_type: Optional[Any] = config["source_type"] if "source_type" in config else None
        else:
