...
from zscaler.zcc.models.predefined_ip_based_apps import PredefinedIPBasedApps
from zscaler.utils import format_url
from zscaler.types import APIResult


class PredefinedIPBasedAppsAPI(APIClient):
    ...
    def get_predefined_ip_based_apps(self) -> APIResult[dict]:
        """
        ...
        Returns:
            :obj:`list`: Retrieves the list of predefined IP-based applications.
        ...
        try:
            ...
            items = response_body.get("appServiceContracts", []) or []
            result = [PredefinedIPBasedApps(item) for item in items if isinstance(item, dict)]
        except Exception as error:
            return (None, response, error)
        return (result, response, None)

    def get_predefined_ip_based_app(self, app_id: str) -> APIResult[dict]:
        """
        ...
        Returns:
            :obj:`list`: Retrieves the predefined IP-based application.
        ...
        api_url = format_url(f"""
            {self._zcc_base_endpoint}
            /predefined-ip-based-apps/{app_id}
        """)
        ...
        response, error = self._request_executor.execute(request, PredefinedIPBasedApps)
        if error:
            return (None, response, error)

        try:
            result = PredefinedIPBasedApps(self.form_response_body(response.get_body()))
        except Exception as error:
            return (None, response, error)
        return (result, response, None)
