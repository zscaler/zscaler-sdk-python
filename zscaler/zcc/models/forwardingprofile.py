...
class ForwardingProfile(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.enable_lwf_driver = config["enableLWFDriver"] \
                if "enableLWFDriver" in config else None
            self.enable_split_vpn_t_n = config["enableSplitVpnTN"] \
                if "enableSplitVpnTN" in config else None
            self.evaluate_trusted_network = config["evaluateTrustedNetwork"] \
                if "evaluateTrustedNetwork" in config else None
            ...
            self.forwarding_profile_zpa_actions = ZscalerCollection.form_list(
                config["forwardingProfileZpaActions"] if "forwardingProfileZpaActions" in config else [], ForwardingProfileZpaActions
            )
            ...
            self.skip_trusted_criteria_match = config["skipTrustedCriteriaMatch"] \
                if "skipTrustedCriteriaMatch" in config else None
...
class ForwardingProfileActions(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            self.dtls_timeout = config["DTLSTimeout"] \
                if "DTLSTimeout" in config else None
            self.tls_timeout = config["TLSTimeout"] \
                if "TLSTimeout" in config else None
            self.udp_timeout = config["UDPTimeout"] \
                if "UDPTimeout" in config else None

            self.action_type = config["actionType"] \
                if "actionType" in config else None
            self.allow_tls_fallback = config["allowTLSFallback"] \
                if "allowTLSFallback" in config else None
            self.block_unreachable_domains_traffic = config["blockUnreachableDomainsTraffic"] \
                if "blockUnreachableDomainsTraffic" in config else None
            self.custom_pac = config["customPac"] \
                if "customPac" in config else None
            self.drop_ipv6_include_traffic_in_t2 = config["dropIpv6IncludeTrafficInT2"] \
                if "dropIpv6IncludeTrafficInT2" in config else None
            self.drop_ipv6_traffic = config["dropIpv6Traffic"] \
                if "dropIpv6Traffic" in config else None
            self.drop_ipv6_traffic_in_ipv6_network = config["dropIpv6TrafficInIpv6Network"] \
                if "dropIpv6TrafficInIpv6Network" in config else None
            self.enable_packet_tunnel = config["enablePacketTunnel"] \
                if "enablePacketTunnel" in config else None
            self.latency_based_zen_enablement = config["latencyBasedZenEnablement"] \
                if "latencyBasedZenEnablement" in config else None
            self.mtu_for_zadapter = config["mtuForZadapter"] \
                if "mtuForZadapter" in config else None
            self.network_type = config["networkType"] \
                if "networkType" in config else None
            self.path_mtu_discovery = config["pathMtuDiscovery"] \
                if "pathMtuDiscovery" in config else None
            self.primary_transport = config["primaryTransport"] \
                if "primaryTransport" in config else None
            self.redirect_web_traffic = config["redirectWebTraffic"] \
                if "redirectWebTraffic" in config else None
            self.system_proxy = config["systemProxy"] if "systemProxy" in config else None
            self.tunnel2_fallback = config["tunnel2Fallback"] \
                if "tunnel2Fallback" in config else None
            self.use_tunnel2_for_proxied_web_traffic = config["useTunnel2ForProxiedWebTraffic"] \
                if "useTunnel2ForProxiedWebTraffic" in config else None
            self.zen_probe_interval = config["zenProbeInterval"] \
                if "zenProbeInterval" in config else None
            self.zen_probe_sample_size = config["zenProbeSampleSize"] \
                if "zenProbeSampleSize" in config else None
            self.zen_threshold_limit = config["zenThresholdLimit"] \
                if "zenThresholdLimit" in config else None
            ...
            if "systemProxyData" in config:
                ...
                self.system_proxy_data = {
                    "bypassProxyForPrivateIP": spd["bypassProxyForPrivateIP"] \
                        if "bypassProxyForPrivateIP" in spd else None,
                    "enableAutoDetect": spd["enableAutoDetect"] \
                        if "enableAutoDetect" in spd else None,
                    "enablePAC": spd["enablePAC"] \
                        if "enablePAC" in spd else None,
                    "enableProxyServer": spd["enableProxyServer"] \
                        if "enableProxyServer" in spd else None,
                    "pacDataPath": spd["pacDataPath"] \
                        if "pacDataPath" in spd else None,
                    "pacURL": spd["pacURL"] \
                        if "pacURL" in spd else None,
                    "performGPUpdate": spd["performGPUpdate"] \
                        if "performGPUpdate" in spd else None,
                    "proxyAction": spd["proxyAction"] \
                        if "proxyAction" in spd else None,
                    "proxyServerAddress": spd["proxyServerAddress"] \
                        if "proxyServerAddress" in spd else None,
                    "proxyServerPort": spd["proxyServerPort"] \
                        if "proxyServerPort" in spd else None
                }
            else:
...
class ForwardingProfileZpaActions(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.action_type = config["actionType"] \
                if "actionType" in config else None
            self.latency_based_server_mt_enablement = config["latencyBasedServerMTEnablement"] \
                if "latencyBasedServerMTEnablement" in config else None
            self.latency_based_zpa_server_enablement = config["latencyBasedZpaServerEnablement"] \
                if "latencyBasedZpaServerEnablement" in config else None
            self.lbs_zpa_probe_interval = config["lbsZpaProbeInterval"] \
                if "lbsZpaProbeInterval" in config else None
            self.lbs_zpa_probe_sample_size = config["lbsZpaProbeSampleSize"] \
                if "lbsZpaProbeSampleSize" in config else None
            self.lbs_zpa_threshold_limit = config["lbsZpaThresholdLimit"] \
                if "lbsZpaThresholdLimit" in config else None
            self.mtu_for_zadapter = config["mtuForZadapter"] \
                if "mtuForZadapter" in config else None
            self.network_type = config["networkType"] \
                if "networkType" in config else None
            ...
            if "partnerInfo" in config:
                ...
                self.partner_info = {
                    "allowTlsFallback": pi["allowTlsFallback"] \
                        if "allowTlsFallback" in pi else None,
                    "mtuForZadapter": pi["mtuForZadapter"] \
                        if "mtuForZadapter" in pi else None,
                    "primaryTransport": pi["primaryTransport"] \
                        if "primaryTransport" in pi else None,
                }
            else:
                self.partner_info = None

            self.primary_transport = config["primaryTransport"] if "primaryTransport" in config else None
            self.send_trusted_network_result_to_zpa = config["sendTrustedNetworkResultToZpa"] \
                if "sendTrustedNetworkResultToZpa" in config else None
