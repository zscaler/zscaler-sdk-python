...
class GetCompanyInfo(ZscalerObject):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if config:
            ...
            self.master_customer_id = config["masterCustomerId"] if "masterCustomerId" in config else None
            self.name = config["name"] if "name" in config else None
            self.business_name = config["businessName"] if "businessName" in config else None
            self.business_contact_number = config["businessContactNumber"] if "businessContactNumber" in config else None
            self.activation_recipient = config["activationRecipient"] if "activationRecipient" in config else None
            self.activation_copy = config["activationCopy"] if "activationCopy" in config else None
            self.mdm_status = config["mdmStatus"] if "mdmStatus" in config else None
            self.send_email = config["sendEmail"] if "sendEmail" in config else None
            self.proxy_enabled = config["proxyEnabled"] if "proxyEnabled" in config else None
            self.zpn_enabled = config["zpnEnabled"] if "zpnEnabled" in config else None
            self.upm_enabled = config["upmEnabled"] if "upmEnabled" in config else None
            self.zad_enabled = config["zadEnabled"] if "zadEnabled" in config else None
            self.enable_deception_for_all = config["enableDeceptionForAll"] if "enableDeceptionForAll" in config else None
            self.dlp_enabled = config["dlpEnabled"] if "dlpEnabled" in config else None
            self.tunnel_protocol_type = config["tunnelProtocolType"] if "tunnelProtocolType" in config else None
            self.secure_agent_basic = config["secureAgentBasic"] if "secureAgentBasic" in config else None
            self.secure_agent_advanced = config["secureAgentAdvanced"] if "secureAgentAdvanced" in config else None
            self.support_admin_email = config["supportAdminEmail"] if "supportAdminEmail" in config else None
            self.support_enabled = config["supportEnabled"] if "supportEnabled" in config else None
            self.fetch_logs_for_admins_enabled = (
                config["fetchLogsForAdminsEnabled"] if "fetchLogsForAdminsEnabled" in config else None
            )
            self.enable_rectify_utils = config["enableRectifyUtils"] if "enableRectifyUtils" in config else None
            self.support_ticket_enabled = config["supportTicketEnabled"] if "supportTicketEnabled" in config else None
            self.disable_logging_controls = config["disableLoggingControls"] if "disableLoggingControls" in config else None
            self.default_auth_type = config["defaultAuthType"] if "defaultAuthType" in config else None
            self.version = config["version"] if "version" in config else None
            self.policy_activation_required = (
                config["policyActivationRequired"] if "policyActivationRequired" in config else None
            )
            self.enable_autofill_username = config["enableAutofillUsername"] if "enableAutofillUsername" in config else None
            self.auto_fill_using_login_hint = config["autoFillUsingLoginHint"] if "autoFillUsingLoginHint" in config else None
            self.dc_service_read_only = config["dcServiceReadOnly"] if "dcServiceReadOnly" in config else None
            self.enable_tunnel_zapp_traffic_toggle = (
            ...
            self.linux_visibility = config["linuxVisibility"] if "linuxVisibility" in config else None
            self.registry_path_for_pac = config["registryPathForPac"] if "registryPathForPac" in config else None
            self.use_pollset_for_socket_reactor = (
                config["usePollsetForSocketReactor"] if "usePollsetForSocketReactor" in config else None
            )
            self.enable_dtls_for_zpa = config["enableDtlsForZpa"] if "enableDtlsForZpa" in config else None
            self.use_v8_js_engine = config["useV8JsEngine"] if "useV8JsEngine" in config else None
            self.disable_parallel_ipv4_and_ipv6 = (
                config["disableParallelIpv4AndIPv6"] if "disableParallelIpv4AndIPv6" in config else None
            )
            self.send64_bit_build = config["send64BitBuild"] if "send64BitBuild" in config else None
            self.use_add_ifscope_route = config["useAddIfscopeRoute"] if "useAddIfscopeRoute" in config else None
            self.use_clear_arp_cache = config["useClearArpCache"] if "useClearArpCache" in config else None
            self.use_dns_priority_ordering = config["useDnsPriorityOrdering"] if "useDnsPriorityOrdering" in config else None
            self.enable_browser_auth = config["enableBrowserAuth"] if "enableBrowserAuth" in config else None
            self.enable_public_api = config["enablePublicAPI"] if "enablePublicAPI" in config else None
            self.disable_reason_visibility = config["disableReasonVisibility"] if "disableReasonVisibility" in config else None
            ...
            self.enable_minimum_device_cleanup_as_one = (
            ...
            self.machine_tunnel_posture = config["machineTunnelPosture"] if "machineTunnelPosture" in config else None
            self.zpa_partner_login = config["zpaPartnerLogin"] if "zpaPartnerLogin" in config else None
            self.proxy_port = config["proxyPort"] if "proxyPort" in config else None
            self.dns_cache_ttl_windows = config["dnsCacheTtlWindows"] if "dnsCacheTtlWindows" in config else None
            self.dns_cache_ttl_mac = config["dnsCacheTtlMac"] if "dnsCacheTtlMac" in config else None
            self.dns_cache_ttl_android = config["dnsCacheTtlAndroid"] if "dnsCacheTtlAndroid" in config else None
            self.dns_cache_ttl_ios = config["dnsCacheTtlIos"] if "dnsCacheTtlIos" in config else None
            self.dns_cache_ttl_linux = config["dnsCacheTtlLinux"] if "dnsCacheTtlLinux" in config else None
            self.zpa_client_cert_exp_in_days = config["zpaClientCertExpInDays"] if "zpaClientCertExpInDays" in config else None
            self.enable_flow_logger = config["enableFlowLogger"] if "enableFlowLogger" in config else None
            self.flow_logging_buffer_limit = config["flowLoggingBufferLimit"] if "flowLoggingBufferLimit" in config else None
            self.flow_logging_time_interval = (
                config["flowLoggingTimeInterval"] if "flowLoggingTimeInterval" in config else None
            )
            self.posture_based_service = config["postureBasedService"] if "postureBasedService" in config else None
            self.enable_posture_based_profile = (
            ...
            self.zia_global_db_url_for_dr = config["ziaGlobalDbUrlForDR"] if "ziaGlobalDbUrlForDR" in config else None
            self.enable_react_ui = config["enableReactUI"] if "enableReactUI" in config else None
            self.launch_react_u_iby_default = config["launchReactUIbyDefault"] if "launchReactUIbyDefault" in config else None
            self.dlp_notification = config["dlpNotification"] if "dlpNotification" in config else None
            self.vpn_gateway_char_limit = config["vpnGatewayCharLimit"] if "vpnGatewayCharLimit" in config else None
            self.device_groups_count = config["deviceGroupsCount"] if "deviceGroupsCount" in config else None
            self.vpn_bypass_refresh_interval = (
                config["vpnBypassRefreshInterval"] if "vpnBypassRefreshInterval" in config else None
            )
            self.dest_include_exclude_char_limit = (
                config["destIncludeExcludeCharLimit"] if "destIncludeExcludeCharLimit" in config else None
            )
            self.ip_v6_support_for_tunnel2 = config["ipV6SupportForTunnel2"] if "ipV6SupportForTunnel2" in config else None
            self.dest_include_exclude_char_limit_for_ipv6 = (
            ...
            self.add_app_bypass_to_vpn_gateway = (
            ...
            self.clear_mup_cache = config["clearMupCache"] if "clearMupCache" in config else None
            self.execute_gpo_update = config["executeGpoUpdate"] if "executeGpoUpdate" in config else None
            self.enable_port_based_zpa_filter = (
            ...
            self.zpa_reauth_enabled = config["zpaReauthEnabled"] if "zpaReauthEnabled" in config else None
            self.zpa_auto_reauth_timeout = config["zpaAutoReauthTimeout"] if "zpaAutoReauthTimeout" in config else None
            self.enable_zpa_auth_user_name = config["enableZpaAuthUserName"] if "enableZpaAuthUserName" in config else None
            self.configure_tunnel2fallback_for_zia = (
                config["configureTunnel2fallbackForZia"] if "configureTunnel2fallbackForZia" in config else None
            )
            self.enable_install_web_view2 = config["enableInstallWebView2"] if "enableInstallWebView2" in config else None
            self.enable_custom_proxy_ports = config["enableCustomProxyPorts"] if "enableCustomProxyPorts" in config else None
            ...
            self.swagger_link = config["swaggerLink"] if "swaggerLink" in config else None
            self.enable_one_id_admin = config["enableOneIdAdmin"] if "enableOneIdAdmin" in config else None
            self.enable_one_id_user = config["enableOneIdUser"] if "enableOneIdUser" in config else None
            self.restrict_admin_access = config["restrictAdminAccess"] if "restrictAdminAccess" in config else None
            self.enable_zia_user_department_sync = (
                config["enableZiaUserDepartmentSync"] if "enableZiaUserDepartmentSync" in config else None
            )
            self.enable_udp_transport_selection = (
                config["enableUDPTransportSelection"] if "enableUDPTransportSelection" in config else None
            )
            self.compute_device_groups_for_zia = (
...
class WebAppConfig(ZscalerObject):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if config:
            self.enable_fips_mode = config["enableFipsMode"] if "enableFipsMode" in config else None
            self.device_cleanup = config["deviceCleanup"] if "deviceCleanup" in config else None
            self.sync_time_hours = config["syncTimeHours"] if "syncTimeHours" in config else None
            self.hide_non_fed_settings = config["hideNonFedSettings"] if "hideNonFedSettings" in config else None
            self.hide_audit_logs = config["hideAuditLogs"] if "hideAuditLogs" in config else None
            self.activate_policy = config["activatePolicy"] if "activatePolicy" in config else None
            self.trusted_network = config["trustedNetwork"] if "trustedNetwork" in config else None
            self.process_postures = config["processPostures"] if "processPostures" in config else None
            self.zpa_reauth = config["zpaReauth"] if "zpaReauth" in config else None
            self.inactive_device_cleanup = config["inactiveDeviceCleanup"] if "inactiveDeviceCleanup" in config else None
            self.zpa_auth_username = config["zpaAuthUsername"] if "zpaAuthUsername" in config else None
            ...
            self.hide_dtls_support_settings = (
                config["hideDTLSSupportSettings"] if "hideDTLSSupportSettings" in config else None
            )
            self.machine_token = config["machineToken"] if "machineToken" in config else None
            self.application_bypass_info = config["applicationBypassInfo"] if "applicationBypassInfo" in config else None
            self.tunnel_two_for_android_devices = (
                config["tunnelTwoForAndroidDevices"] if "tunnelTwoForAndroidDevices" in config else None
            )
            self.tunnel_two_fori_os_devices = config["tunnelTwoForiOSDevices"] if "tunnelTwoForiOSDevices" in config else None
            self.ownership_variable_posture = (
                config["ownershipVariablePosture"] if "ownershipVariablePosture" in config else None
            )
            self.block_unreachable_domains_traffic_flag = (
                config["blockUnreachableDomainsTrafficFlag"] if "blockUnreachableDomainsTrafficFlag" in config else None
            )
            self.prioritize_ipv4_over_ipv6 = config["prioritizeIPv4OverIpv6"] if "prioritizeIPv4OverIpv6" in config else None
            self.crowd_strike_zta_score_visibility = (
                config["crowdStrikeZTAScoreVisibility"] if "crowdStrikeZTAScoreVisibility" in config else None
            )
            self.notification_for_zpa_reauth_visibility = (
                config["notificationForZPAReauthVisibility"] if "notificationForZPAReauthVisibility" in config else None
            )
            self.crl_check_visibility_flag = config["crlCheckVisibilityFlag"] if "crlCheckVisibilityFlag" in config else None
            self.dedicated_proxy_ports_visibility = (
                config["dedicatedProxyPortsVisibility"] if "dedicatedProxyPortsVisibility" in config else None
            )
            self.remote_fetch_logs = config["remoteFetchLogs"] if "remoteFetchLogs" in config else None
            self.ms_defender_posture_visibility = (
                config["msDefenderPostureVisibility"] if "msDefenderPostureVisibility" in config else None
            )
            self.exit_password_visibility = config["exitPasswordVisibility"] if "exitPasswordVisibility" in config else None
            self.collect_zdx_location_visibility = (
                config["collectZdxLocationVisibility"] if "collectZdxLocationVisibility" in config else None
            )
            self.use_v8_js_engine_visibility = (
                config["useV8JsEngineVisibility"] if "useV8JsEngineVisibility" in config else None
            )
            self.zdx_disable_password_visibility = (
                config["zdxDisablePasswordVisibility"] if "zdxDisablePasswordVisibility" in config else None
            )
            self.zad_disable_password_visibility = (
                config["zadDisablePasswordVisibility"] if "zadDisablePasswordVisibility" in config else None
            )
            self.zpa_disable_password_visibility = (
                config["zpaDisablePasswordVisibility"] if "zpaDisablePasswordVisibility" in config else None
            )
            self.default_protocol_for_zpa = config["defaultProtocolForZPA"] if "defaultProtocolForZPA" in config else None
            self.drop_ipv6_traffic_visibility = (
                config["dropIpv6TrafficVisibility"] if "dropIpv6TrafficVisibility" in config else None
            )
            self.mac_cache_system_proxy_visibility = (
            ...
            ["authServerAllowListImprivataVisibility"] if "authServerAllowListImprivataVisibility" in config else None
            ...
            self.disable_reason_visibility = config["disableReasonVisibility"] if "disableReasonVisibility" in config else None
            self.follow_routing_table_visibility = (
                config["followRoutingTableVisibility"] if "followRoutingTableVisibility" in config else None
            )
            self.zia_device_posture_visibility = (
                config["ziaDevicePostureVisibility"] if "ziaDevicePostureVisibility" in config else None
            )
            self.use_custom_dns = config["useCustomDNS"] if "useCustomDNS" in config else None
            self.use_default_adapter_for_dns_visibility = config
            ["useDefaultAdapterForDNSVisibility"] if "useDefaultAdapterForDNSVisibility" in config else None
            self.t2_fallback_block_all_traffic_and_tls_fallback = config
            ["t2FallbackBlockAllTrafficAndTlsFallback"] if "t2FallbackBlockAllTrafficAndTlsFallback" in config else None
            ...
            ["grantAccessToZscalerLogFolderVisibility"] if "grantAccessToZscalerLogFolderVisibility" in config else None
            self.admin_management_visibility = (
            ...
            ["dynamicZPAServiceEdgeAssignmenttVisibility"] if "dynamicZPAServiceEdgeAssignmenttVisibility" in config else None
            self.custom_proxy_ports_visibility = (
            ...
            self.retry_after_in_seconds = config["retryAfterInSeconds"] if "retryAfterInSeconds" in config else None
            self.azure_a_d_posture_visibility = (
                config["azureADPostureVisibility"] if "azureADPostureVisibility" in config else None
            )
            self.server_cert_posture_visibility = (
                config["serverCertPostureVisibility"] if "serverCertPostureVisibility" in config else None
            )
            self.auto_fill_using_login_hint_visibility = (
            ...
            ["sendDefaultPolicyForInvalidPolicyToken"] if "sendDefaultPolicyForInvalidPolicyToken" in config else None
            self.enable_zcc_password_settings = (
                config["enableZccPasswordSettings"] if "enableZccPasswordSettings" in config else None
            )
            self.cli_password_expiry_minutes = (
                config["cliPasswordExpiryMinutes"] if "cliPasswordExpiryMinutes" in config else None
            )
            self.sso_using_windows_primary_account = (
...
class DevicePostureFrequency(ZscalerObject):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if config:
            self.posture_id = config["postureId"] if "postureId" in config else None
            self.posture_name = config["postureName"] if "postureName" in config else None
            self.ios_value = config["iosValue"] if "iosValue" in config else None
            self.android_value = config["androidValue"] if "androidValue" in config else None
            self.windows_value = config["windowsValue"] if "windowsValue" in config else None
            self.mac_value = config["macValue"] if "macValue" in config else None
            self.linux_value = config["linuxValue"] if "linuxValue" in config else None
            self.default_value = config["defaultValue"] if "defaultValue" in config else None
        else:
