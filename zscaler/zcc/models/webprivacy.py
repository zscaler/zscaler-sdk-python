...
class WebPrivacy(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.collect_machine_hostname = config["collectMachineHostname"] \
                if "collectMachineHostname" in config else None
            self.collect_user_info = config["collectUserInfo"] \
                if "collectUserInfo" in config else None
            self.collect_zdx_location = config["collectZdxLocation"] \
                if "collectZdxLocation" in config else None
            self.disable_crashlytics = config["disableCrashlytics"] \
                if "disableCrashlytics" in config else None
            self.enable_packet_capture = config["enablePacketCapture"] \
                if "enablePacketCapture" in config else None
            self.export_logs_for_non_admin = config["exportLogsForNonAdmin"] \
                if "exportLogsForNonAdmin" in config else None
            self.grant_access_to_zscaler_log_folder = config["grantAccessToZscalerLogFolder"] \
                if "grantAccessToZscalerLogFolder" in config else None
            self.id = config["id"] \
                if "id" in config else None
            self.override_t2_protocol_setting = config["overrideT2ProtocolSetting"] \
                if "overrideT2ProtocolSetting" in config else None
            self.restrict_remote_packet_capture = config["restrictRemotePacketCapture"] \
                if "restrictRemotePacketCapture" in config else None
        else:
            self.active = None
