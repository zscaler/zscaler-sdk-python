    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            if "lastModifiedBy" in config:
                if isinstance(config["lastModifiedBy"], common.Common):
                    self.last_modified_by = config["lastModifiedBy"]
                elif config["lastModifiedBy"] is not None:
                    self.last_modified_by = common.Common(config["lastModifiedBy"])
                else:
            ...
            if "partner" in config:
                if isinstance(config["partner"], common.Common):
                    self.partner = config["partner"]
                elif config["partner"] is not None:
                    self.partner = common.Common(config["partner"])
                else:
                    self.partner = None
            else:
                self.partner = None
