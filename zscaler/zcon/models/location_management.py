...
from zscaler.zcon.models import traffic_vpn_credentials as vpn_credentials
...
class LocationManagement(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.override_up_bandwidth = config["overrideUpBandwidth"] \
                if "overrideUpBandwidth" in config else 0
            self.override_dn_bandwidth = config["overrideDnBandwidth"] \
                if "overrideDnBandwidth" in config else 0
            self.shared_up_bandwidth = config["sharedUpBandwidth"] \
                if "sharedUpBandwidth" in config else 0
            self.shared_down_bandwidth = config["sharedDownBandwidth"] \
                if "sharedDownBandwidth" in config else 0
            self.unused_up_bandwidth = config["unusedUpBandwidth"] \
                if "unusedUpBandwidth" in config else 0
            self.unused_dn_bandwidth = config["unusedDnBandwidth"] \
                if "unusedDnBandwidth" in config else 0
            ...
            self.ipv6_dns64_prefix = config["ipv6Dns64Prefix"] \
                if "ipv6Dns64Prefix" in config else None

            self.managed_by = config["managedBy"] \
                if "managedBy" in config else None

            self.multi_tenant_vpn_credential = config["multiTenantVpnCredential"] \
                if "multiTenantVpnCredential" in config else None

            self.vpc_info = config["vpcInfo"] \
                if "vpcInfo" in config else None

            self.public_cloud_account_id = config["publicCloudAccountId"] \
                if "publicCloudAccountId" in config else None
            ...
            self.virtual_zens = ZscalerCollection.form_list(
                config["virtualZens"] if "virtualZens" in config else [], str
            )
