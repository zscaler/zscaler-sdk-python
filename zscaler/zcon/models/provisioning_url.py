...
from zscaler.zcon.models import location_templates as location_templates
from zscaler.zcon.models import common as common
from zscaler.zcon.models import ecgroup as ecgroup
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            if "location" in config:
                if isinstance(config["location"], common.Common):
                    self.location = config["location"]
                elif config["location"] is not None:
                    self.location = common.Common(config["location"])
                else:
                    self.location = None
            else:
                self.location = None
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            if "cloudProvider" in config:
                if isinstance(config["cloudProvider"], common.Common):
                    self.cloud_provider = config["cloudProvider"]
                elif config["cloudProvider"] is not None:
                    self.cloud_provider = common.Common(config["cloudProvider"])
                else:
