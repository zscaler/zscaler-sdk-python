...
class IpRanges(ZscalerObject):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if config:
            self.available_ips = config["availableIps"] if "availableIps" in config else None
            ...
            self.ip_range_begin = config["ipRangeBegin"] if "ipRangeBegin" in config else None
            self.ip_range_end = config["ipRangeEnd"] if "ipRangeEnd" in config else None
            self.is_deleted = config["isDeleted"] if "isDeleted" in config else None
            self.latitude_in_db = config["latitudeInDb"] if "latitudeInDb" in config else None
            self.location = config["location"] if "location" in config else None
            self.location_hint = config["locationHint"] if "locationHint" in config else None
            self.longitude_in_db = config["longitudeInDb"] if "longitudeInDb" in config else None
            self.modified_by = config["modifiedBy"] if "modifiedBy" in config else None
            self.modified_time = config["modifiedTime"] if "modifiedTime" in config else None
            self.name = config["name"] if "name" in config else None
            self.sccm_flag = config["sccmFlag"] if "sccmFlag" in config else None
            self.subnet_cidr = config["subnetCidr"] if "subnetCidr" in config else None
            self.total_ips = config["totalIps"] if "totalIps" in config else None
            self.used_ips = config["usedIps"] if "usedIps" in config else None
        else:
