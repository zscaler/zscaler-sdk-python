...
class PolicyGroupSet(ZscalerObject):
    """
    A class for PolicyGroupSet objects.
    """

    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        """
        Initialize the PolicyGroupSet model based on API response.
        ...
        if config:
            ...
            self.custom_policy_group_gids = ZscalerCollection.form_list(
                config["customPolicyGroupGids"] if "customPolicyGroupGids" in config else [], int
            )
        else:
