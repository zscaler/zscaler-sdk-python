    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.admin_sp_signing_cert_id = config["adminSpSigningCertId"] \
                if "adminSpSigningCertId" in config else None
            self.auto_provision = config["autoProvision"] \
                if "autoProvision" in config else None
            ...
            self.disable_saml_based_policy = config["disableSamlBasedPolicy"] \
                if "disableSamlBasedPolicy" in config else None
            self.enable_arbitrary_auth_domains = config["enableArbitraryAuthDomains"] \
                if "enableArbitraryAuthDomains" in config else None
            self.enable_scim_based_policy = config["enableScimBasedPolicy"] \
                if "enableScimBasedPolicy" in config else None
            self.enabled = config["enabled"] \
                if "enabled" in config else None
            self.force_auth = config["forceAuth"] \
                if "forceAuth" in config else None
            self.iam_idp_id = config["iamIdpId"] \
                if "iamIdpId" in config else None
            self.idp_entity_id = config["idpEntityId"] \
                if "idpEntityId" in config else None
            self.login_hint = config["loginHint"] \
                if "loginHint" in config else None
            self.login_name_attribute = config["loginNameAttribute"] \
                if "loginNameAttribute" in config else None
            self.login_url = config["loginUrl"] \
                if "loginUrl" in config else None
            self.migration_detail = config["migrationDetail"] \
                if "migrationDetail" in config else None
            ...
            self.one_identity_enabled = config["oneIdentityEnabled"] \
                if "oneIdentityEnabled" in config else None
            self.reauth_on_user_update = config["reauthOnUserUpdate"] \
                if "reauthOnUserUpdate" in config else None
            self.redirect_binding = config["redirectBinding"] \
                if "redirectBinding" in config else None
            self.scim_enabled = config["scimEnabled"] \
                if "scimEnabled" in config else None
            self.scim_service_provider_endpoint = config["scimServiceProviderEndpoint"] \
                if "scimServiceProviderEndpoint" in config else None
            self.scim_shared_secret_exists = config["scimSharedSecretExists"] \
                if "scimSharedSecretExists" in config else None
            self.sign_saml_request = config["signSamlRequest"] \
                if "signSamlRequest" in config else None

            self.use_custom_sp_metadata = config["useCustomSPMetadata"] \
                if "useCustomSPMetadata" in config else None

            self.user_sp_signing_cert_id = config["userSpSigningCertId"] \
                if "userSpSigningCertId" in config else None

            self.domain_list = ZscalerCollection.form_list(
                config["domainList"] if "domainList" in config else [], str
            )
...
class ServiceProvider(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        super().__init__(config)
        if config:
            self.certificate_url = config["certificateUrl"] \
                if "certificateUrl" in config else None
            self.sp_base_url = config["spBaseUrl"] \
                if "spBaseUrl" in config else None
            self.sp_entity_id = config["spEntityId"] \
                if "spEntityId" in config else False
