...
from zscaler.aiguard.models.policy_detection import (
...
class PolicyDetectionAPI(APIClient):
    ...
    .. warning::
        ...
        These endpoints are **legacy only**. ``/v1/detection/execute-policy`` and
        ``/v1/detection/resolve-and-execute-policy`` are not exposed through OneAPI,
        so this interface is reached with
        :class:`~zscaler.oneapi_client.LegacyAIGuardClient` (AI Guard API key against
        ``https://api.<cloud>.zseclipse.net``) rather than ``ZscalerClient``.
