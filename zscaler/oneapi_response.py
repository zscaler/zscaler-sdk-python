...
#         # If service is ZPA and user did not specify a pagesize, set it to max=500
#         if self._service_type == "ZPA" and "pagesize" not in self._params:
#             self._params["pagesize"] = self.SERVICE_PAGE_LIMITS["ZPA"]["max"]
...
#     def get_all_pages_results(self, search_callback=None):
#         self._items_fetched = 0
#         self._pages_fetched = 0
#         self._page = 1
...
#                 if search_callback(item):
#                     logger.debug("Search item found on first page, stopping pagination.")
#                     return all_results

#         prev_results = self._list.copy()  # Track previous page results
...
#             # Additional check for ZIA to prevent infinite loop:
#             # If we got the same results as last time, break to avoid looping indefinitely.
#             if self._service_type == "ZIA" and next_page_results == prev_results:
#                 logger.debug("No new data returned for ZIA, stopping pagination to avoid infinite loop.")
#                 break
...
#             # Update prev_results for the next iteration
#             prev_results = next_page_results.copy()

#             if search_callback:
#                 for item in next_page_results:
#                     if search_callback(item):
#                         logger.debug("Search item found, stopping pagination.")
#                         return all_results
...
#             return has_next
#         else:
#             # For ZIA/ZCC, continue if the current list is not empty
