...
class Devices(ZscalerObject):
    ...
    This lightweight device reference block is embedded in policy/rule payloads
    (firewall, URL filtering, SSL inspection, etc.) and is distinct from the
    ``RegisteredDevice`` model returned by the registered-devices endpoints.
    """
