...
class ConfigAudit(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            self.overall_grade = config["overallGrade"] \
                if "overallGrade" in config else None
            self.high_availability_grade = config["highAvailabilityGrade"] \
                if "highAvailabilityGrade" in config else None
            self.user_performance_grade = config["userPerformanceGrade"] \
                if "userPerformanceGrade" in config else None
            self.other_grade = config["otherGrade"] \
                if "otherGrade" in config else None
            self.gre_tunnel_grade = config["greTunnelGrade"] \
                if "greTunnelGrade" in config else None
            self.pac_file_grade = config["pacFileGrade"] \
                if "pacFileGrade" in config else None
            self.auth_frequency_grade = config["authFrequencyGrade"] \
                if "authFrequencyGrade" in config else None
            self.pacfile_size_grade = config["pacfileSizeGrade"] \
                if "pacfileSizeGrade" in config else None
            self.office365_flag = config["office365Flag"] \
                if "office365Flag" in config else None
            self.office365_grade = config["office365Grade"] \
                if "office365Grade" in config else None
            self.ip_visibility_grade = config["ipVisibilityGrade"] \
                if "ipVisibilityGrade" in config else None
            self.report_timestamp = config["reportTimestamp"] \
                if "reportTimestamp" in config else None
            self.data_present = config["dataPresent"] \
                if "dataPresent" in config else None
            self.gre_tunnel_recommended_configuration = config["greTunnelRecommendedConfiguration"] \
                if "greTunnelRecommendedConfiguration" in config else None
            self.pac_file_recommended_configuration = config["pacFileRecommendedConfiguration"] \
                if "pacFileRecommendedConfiguration" in config else None
            self.auth_frequency_recommended_configuration = config["authFrequencyRecommendedConfiguration"] \
                if "authFrequencyRecommendedConfiguration" in config else None
            self.pac_file_size_recommended_configuration = config["pacFileSizeRecommendedConfiguration"] \
                if "pacFileSizeRecommendedConfiguration" in config else None
            self.office365_recommended_configuration = config["office365RecommendedConfiguration"] \
                if "office365RecommendedConfiguration" in config else None
            self.ip_visibility_recommended_configuration = config["ipVisibilityRecommendedConfiguration"] \
                if "ipVisibilityRecommendedConfiguration" in config else None
...
class IPVisibility(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            self.total_gre_locations = config["totalGreLocations"]\
                if "totalGreLocations" in config else None
            self.recommendation = config["recommendation"]\
                if "recommendation" in config else None
            self.details = config["details"]\
                if "details" in config else None
...
class PacFile(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            self.total_pac_files = config["totalPacFiles"]\
                if "totalPacFiles" in config else None
...
class PacWithStaticIPs(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            self.pac_file_name = config["pacFileName"]\
                if "pacFileName" in config else None
