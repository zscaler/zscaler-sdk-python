...
class EndUserNotification(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            self.aup_frequency = config["aupFrequency"] \
            ...
            self.display_comp_logo = config["displayCompLogo"] \
                if "displayCompLogo" in config else False
            self.custom_text = config["customText"] \
                if "customText" in config else None
            self.url_cat_review_enabled = config["urlCatReviewEnabled"] \
                if "urlCatReviewEnabled" in config else False
            self.url_cat_review_submit_to_security_cloud = config["urlCatReviewSubmitToSecurityCloud"] \
                if "urlCatReviewSubmitToSecurityCloud" in config else False
            self.url_cat_review_custom_location = config["urlCatReviewCustomLocation"] \
                if "urlCatReviewCustomLocation" in config else None
            self.url_cat_review_text = config["urlCatReviewText"] \
                if "urlCatReviewText" in config else None
            self.security_review_enabled = config["securityReviewEnabled"] \
                if "securityReviewEnabled" in config else False
            self.security_review_submit_to_security_cloud = config["securityReviewSubmitToSecurityCloud"] \
                if "securityReviewSubmitToSecurityCloud" in config else False
            self.security_review_custom_location = config["securityReviewCustomLocation"] \
                if "securityReviewCustomLocation" in config else None
            self.security_review_text = config["securityReviewText"] \
                if "securityReviewText" in config else None
            self.web_dlp_review_enabled = config["webDlpReviewEnabled"] \
                if "webDlpReviewEnabled" in config else False
            self.web_dlp_review_submit_to_security_cloud = config["webDlpReviewSubmitToSecurityCloud"] \
                if "webDlpReviewSubmitToSecurityCloud" in config else False
            self.web_dlp_review_custom_location = config["webDlpReviewCustomLocation"] \
                if "webDlpReviewCustomLocation" in config else None
            self.web_dlp_review_text = config["webDlpReviewText"] \
                if "webDlpReviewText" in config else None
            self.redirect_url = config["redirectUrl"] \
                if "redirectUrl" in config else None
            self.support_email = config["supportEmail"] \
                if "supportEmail" in config else None
            self.support_phone = config["supportPhone"] \
                if "supportPhone" in config else None
            self.org_policy_link = config["orgPolicyLink"] \
                if "orgPolicyLink" in config else None
            self.caution_again_after = config["cautionAgainAfter"] \
                if "cautionAgainAfter" in config else None
            self.caution_per_domain = config["cautionPerDomain"] \
                if "cautionPerDomain" in config else False
            self.caution_custom_text = config["cautionCustomText"] \
                if "cautionCustomText" in config else None
            self.idp_proxy_notification_text = config["idpProxyNotificationText"] \
                if "idpProxyNotificationText" in config else None
            self.quarantine_custom_notification_text = config["quarantineCustomNotificationText"] \
                if "quarantineCustomNotificationText" in config else None
        else:
