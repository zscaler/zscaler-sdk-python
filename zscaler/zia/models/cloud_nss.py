...
class NssFeeds(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.feed_status = config["feedStatus"] \
                if "feedStatus" in config else None
            self.nss_log_type = config["nssLogType"] \
                if "nssLogType" in config else None
            self.nss_feed_type = config["nssFeedType"] \
                if "nssFeedType" in config else None
            self.feed_output_format = config["feedOutputFormat"] \
                if "feedOutputFormat" in config else None
            self.user_obfuscation = config["userObfuscation"] \
                if "userObfuscation" in config else None
            ...
            self.eps_rate_limit = config["epsRateLimit"] \
                if "epsRateLimit" in config else None
            self.json_array_toggle = config["jsonArrayToggle"] \
                if "jsonArrayToggle" in config else None
            self.siem_type = config["siemType"] \
                if "siemType" in config else None
            self.max_batch_size = config["maxBatchSize"] \
                if "maxBatchSize" in config else None
            self.connection_u_r_l = config["connectionURL"] \
                if "connectionURL" in config else None
            self.authentication_token = config["authenticationToken"] \
                if "authenticationToken" in config else None
            self.connection_headers = ZscalerCollection.form_list(
                config["connectionHeaders"] if "connectionHeaders" in config else [], str
            )
            self.last_success_full_test = config["lastSuccessFullTest"] \
                if "lastSuccessFullTest" in config else None
            self.test_connectivity_code = config["testConnectivityCode"] \
                if "testConnectivityCode" in config else None
            self.base64_encoded_certificate = config["base64EncodedCertificate"] \
                if "base64EncodedCertificate" in config else None
            self.nss_type = config["nssType"] \
                if "nssType" in config else None
            self.client_id = config["clientId"] \
                if "clientId" in config else None
            self.client_secret = config["clientSecret"] \
                if "clientSecret" in config else None
            self.authentication_url = config["authenticationUrl"] \
                if "authenticationUrl" in config else None
            self.grant_type = config["grantType"] \
                if "grantType" in config else None
            self.scope = config["scope"] \
                if "scope" in config else None
            self.oauth_authentication = config["oauthAuthentication"] \
                if "oauthAuthentication" in config else None
            self.server_ips = ZscalerCollection.form_list(
                config["serverIps"] if "serverIps" in config else [], str
            )
            self.client_ips = ZscalerCollection.form_list(
                config["clientIps"] if "clientIps" in config else [], str
            )
            ...
            self.dns_responses = ZscalerCollection.form_list(
                config["dnsResponses"] if "dnsResponses" in config else [], str
            )
            self.durations = ZscalerCollection.form_list(
                config["durations"] if "durations" in config else [], str
            )
            self.dns_actions = ZscalerCollection.form_list(
                config["dnsActions"] if "dnsActions" in config else [], str
            )
            self.firewall_logging_mode = config["firewallLoggingMode"] \
                if "firewallLoggingMode" in config else None
            ...
            self.countries = ZscalerCollection.form_list(
                config["countries"] if "countries" in config else [], str
            )
            ...
            self.action_filter = config["actionFilter"] \
                if "actionFilter" in config else None
            self.email_dlp_policy_action = config["emailDlpPolicyAction"] \
                if "emailDlpPolicyAction" in config else None
            self.direction = config["direction"] \
                if "direction" in config else None
            self.event = config["event"] \
                if "event" in config else None
            ...
            self.user_agents = ZscalerCollection.form_list(
                config["userAgents"] if "userAgents" in config else [], str
            )
            self.request_methods = ZscalerCollection.form_list(
                config["requestMethods"] if "requestMethods" in config else [], str
            )
            self.casb_severity = ZscalerCollection.form_list(
                config["casbSeverity"] if "casbSeverity" in config else [], str
            )
            ...
            self.casb_action = ZscalerCollection.form_list(
                config["casbAction"] if "casbAction" in config else [], str
            )
            self.casb_tenant = ZscalerCollection.form_list(
                config["casbTenant"] if "casbTenant" in config else [], str
            )
            ...
            self.malware_names = ZscalerCollection.form_list(
                config["malwareNames"] if "malwareNames" in config else [], str
            )
            self.url_classes = ZscalerCollection.form_list(
                config["urlClasses"] if "urlClasses" in config else [], str
            )
            ...
            self.nat_actions = ZscalerCollection.form_list(
                config["natActions"] if "natActions" in config else [], str
            )
            ...
            self.tunnel_types = ZscalerCollection.form_list(
                config["tunnelTypes"] if "tunnelTypes" in config else [], str
            )
            self.alerts = ZscalerCollection.form_list(
                config["alerts"] if "alerts" in config else [], str
            )
            self.object_type = ZscalerCollection.form_list(
                config["objectType"] if "objectType" in config else [], str
            )
            self.activity = ZscalerCollection.form_list(
                config["activity"] if "activity" in config else [], str
            )
            self.object_type1 = ZscalerCollection.form_list(
                config["objectType1"] if "objectType1" in config else [], str
            )
            self.object_type2 = ZscalerCollection.form_list(
                config["objectType2"] if "objectType2" in config else [], str
            )
            self.end_point_d_l_p_log_type = ZscalerCollection.form_list(
                config["endPointDLPLogType"] if "endPointDLPLogType" in config else [], str
            )
            self.email_d_l_p_log_type = ZscalerCollection.form_list(
            ...
            self.casb_file_type = ZscalerCollection.form_list(
                config["casbFileType"] if "casbFileType" in config else [], str
            )
            ...
            self.sender_name = ZscalerCollection.form_list(
                config["senderName"] if "senderName" in config else [], str
            )
            self.buckets = ZscalerCollection.form_list(
                config["buckets"] if "buckets" in config else [], str
            )
            ...
            self.message_size = ZscalerCollection.form_list(
                config["messageSize"] if "messageSize" in config else [], str
            )
            self.file_sizes = ZscalerCollection.form_list(
                config["fileSizes"] if "fileSizes" in config else [], str
            )
            self.request_sizes = ZscalerCollection.form_list(
                config["requestSizes"] if "requestSizes" in config else [], str
            )
            ...
            self.inbound_bytes = ZscalerCollection.form_list(
                config["inBoundBytes"] if "inBoundBytes" in config else [], str
            )
            self.outbound_bytes = ZscalerCollection.form_list(
                config["outBoundBytes"] if "outBoundBytes" in config else [], str
            )
            self.download_time = ZscalerCollection.form_list(
                config["downloadTime"] if "downloadTime" in config else [], str
            )
            self.scan_time = ZscalerCollection.form_list(
                config["scanTime"] if "scanTime" in config else [], str
            )
            ...
            self.tunnel_ips = ZscalerCollection.form_list(
                config["tunnelIps"] if "tunnelIps" in config else [], str
            )
            self.internal_ips = ZscalerCollection.form_list(
                config["internalIps"] if "internalIps" in config else [], str
            )
            ...
            self.audit_log_type = ZscalerCollection.form_list(
                config["auditLogType"] if "auditLogType" in config else [], str
            )
            self.project_name = ZscalerCollection.form_list(
                config["projectName"] if "projectName" in config else [], str
            )
            self.repo_name = ZscalerCollection.form_list(
                config["repoName"] if "repoName" in config else [], str
            )
            self.object_name = ZscalerCollection.form_list(
                config["objectName"] if "objectName" in config else [], str
            )
            self.channel_name = ZscalerCollection.form_list(
                config["channelName"] if "channelName" in config else [], str
            )
            self.file_source = ZscalerCollection.form_list(
                config["fileSource"] if "fileSource" in config else [], str
            )
            self.file_name = ZscalerCollection.form_list(
                config["fileName"] if "fileName" in config else [], str
            )
            ...
            self.referer_urls = ZscalerCollection.form_list(
                config["refererUrls"] if "refererUrls" in config else [], str
            )
            self.host_names = ZscalerCollection.form_list(
                config["hostNames"] if "hostNames" in config else [], str
            )
            self.full_urls = ZscalerCollection.form_list(
                config["fullUrls"] if "fullUrls" in config else [], str
            )
            self.threat_names = ZscalerCollection.form_list(
                config["threatNames"] if "threatNames" in config else [], str
            )
            ...
            self.object_type1 = []
            self.object_type2 = []
            self.end_point_d_l_p_log_type = []
            self.email_d_l_p_log_type = []
    ...
    def request_format(self):
        ...
        current_obj_format = {
            ...
            "connectionURL": self.connection_u_r_l,
            ...
            "endPointDLPLogType": self.end_point_d_l_p_log_type,
            "emailDLPLogType": self.email_d_l_p_log_type,
...
class NSSTestConnectivity(ZscalerObject):
    ...
    def __init__(self, config=None):
        ...
        if config:
            ...
            self.last_success_full_test = config["lastSuccessFullTest"] \
                if "lastSuccessFullTest" in config else None
            self.test_connectivity_status = config["testConnectivityStatus"] \
                if "testConnectivityStatus" in config else None
            self.test_connectivity_code = config["testConnectivityCode"] \
                if "testConnectivityCode" in config else None
            self.base64_encoded_certificate = config["base64EncodedCertificate"] \
                if "base64EncodedCertificate" in config else None
            ...
            self.scope = config["scope"] \
                if "scope" in config else None
            self.oauth_authentication = config["oauthAuthentication"] \
                if "oauthAuthentication" in config else False
        else:
            self.siem_type = None
            self.max_batch_size = None
            self.connection_u_r_l = None
