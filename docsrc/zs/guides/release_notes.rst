...
(`#404 <https://github.com/zscaler/zscaler-sdk-python/pull/404>`_) - Fix OAuth authentication to respect proxy configuration. OAuth requests now use the same proxy settings as regular API calls, resolving issues where authentication would fail when proxy was configured.
